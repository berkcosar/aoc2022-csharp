...
namespace Day14
{
    public class Q1 : BaseQuestion
    {
        public override object Run()
        {
            ...
            int maxRowLength = maxRow + 2;
            int maxColumnLength = (maxColumn - minColumn) + 3;
            int columnIndexOffset = minColumn - 1;
            ...
            for (int row = 0; row < maxRowLength; row++)
            {
                ...
                {
                    if (rockCoordinates.Contains((col + columnIndexOffset, row)))
                    {
                        cave[row, col] = '#';
                    }
                    else
            ...
            while(true)
            {
                ...
                if(sandNew.row == cave.GetLength(0)-1 || sandNew.column == 0 || sandNew.column == cave.GetLength(1)-1)
                {
                    break;
                }
        ...
        private (int column, int row) FallNext(char[,] cave ,(int column, int row) sand)
        {
            ...
            if(returnVal.row == cave.GetLength(0)-1 || returnVal.column == 0 || returnVal.column == cave.GetLength(1)-1)
            {
                return returnVal;
            }
            ...
            {
                if(cave[returnVal.row+1,returnVal.column -1] == '.')
                {
                    return FallNext(cave, (returnVal.column-1, returnVal.row+1));
                }
                else if(cave[returnVal.row+1,returnVal.column +1] == '.')
                {
                     return FallNext(cave, (returnVal.column+1, returnVal.row+1));
                }
                else
