    ...
    public class Q2:BaseQuestion
    {
        public override object Run()
        {
            ...
            for(int i = emptyLineIndex+1; i< lines.Length;i++)
            {
                ...
                char[] cratesToMove = new char[rearrangementCommand.numOfItemsToMove];
                for(int m =0;m< rearrangementCommand.numOfItemsToMove; m++)
                {
                    cratesToMove[m] = stacks[rearrangementCommand.sourceStackIndex].Pop();
                }
                for(int m =rearrangementCommand.numOfItemsToMove-1; m>-1; m--)
                {
                    stacks[rearrangementCommand.targetStackIndex].Push(cratesToMove[m]);
                }
